...
module.exports.login = async (req, res) => {
  ...
  //generating a new token to set in LS of the admin
  if (db()) {
    try {
      const password = md5(req.query.senha);
      const cpf = req.query.cpf;

      const verifyingAdmin = await adminModel.findOne({ senha: password });

      if(!verifyingAdmin){
          res.status(401).json({message: "Seu usuário nao existe."})
          return
      }
      ...
      if(!token){
          console.log("Deu erro ao gerar o token")
          return
      }
  ...
  if (db()) {
    try {
      ...
      if (!(login && senha && cpf && rg)) {
        res
          .status(400)
          .json({ error: "login, senha, cpf e rg são necessários" });
        return;
      }
      ...
      if (verifyCpf) {
        res
          .status(409)
          .json({
            error: "Usuário com este cpf já existe. Apenas um cadastro por cpf",
          });
        return;
      }
